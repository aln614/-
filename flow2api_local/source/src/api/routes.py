...
async def _load_video_bytes_from_uri(uri: str) -> bytes:
    if not uri:
        raise HTTPException(status_code=400, detail="Video URI cannot be empty")
    if uri.startswith("data:video/"):
        _, video_bytes = _decode_data_url(uri)
        return video_bytes
    if uri.startswith("http://") or uri.startswith("https://"):
        async with AsyncSession() as session:
            response = await session.get(uri, timeout=120, impersonate="chrome120", verify=False)
            if response.status_code == 200 and response.content:
                return response.content
...
) -> tuple[str, List[bytes], Optional[str], Optional[bytes]]:
    """Extract prompt, images, and optional video_media_id from messages.

    Returns:
        (prompt, images, video_media_id, video_bytes)
    ...
    if isinstance(content, str):
        ...
        for item in content:
            ...
            if item_type == "text":
                ...
                video_bytes = await _load_video_bytes_from_uri(video_url)

    prompt = "\n".join(part for part in prompt_parts if part).strip()
    return prompt, images, video_media_id, video_bytes
...
async def _normalize_openai_request(
    ...
    if request.messages:
        prompt, images, video_media_id, video_bytes = await _extract_prompt_and_images_from_openai_messages(
            request.messages
        )
...
async def captcha_websocket_endpoint(websocket: WebSocket):
    ...
    def _is_trusted_local_ws_client() -> bool:
        host = getattr(websocket.client, "host", None) if websocket.client else None
        if not host:
            return False
        try:
    ...
    if (not api_key or not AuthManager.verify_api_key(api_key)) and not _is_trusted_local_ws_client():
        await websocket.close(code=1008)
        return

    handler = _ensure_generation_handler()
    service = await ExtensionCaptchaService.get_instance(getattr(handler, "db", None))
