...
class TokenManager:
    ...
    @staticmethod
    def _is_transient_captcha_error(error_message: str = "") -> bool:
        """reCAPTCHA risk-control failures do not mean the account token is invalid."""
        message = str(error_message or "").lower()
        return "recaptcha" in message or any(marker in message for marker in (
            "public_error_unusual_activity",
            "failed to obtain captcha",
            "failed to obtain recaptcha",
        ))

    async def record_error(self, token_id: int, error_message: str = ""):
        """Record token error and auto-disable only for persistent token failures."""
        await self.db.increment_token_stats(token_id, "error")

        # Browser/IP risk control is temporary. The captcha service already recycles
        # its browser profile for these failures, so permanently disabling the only
        # account here turns a recoverable error into an endless 503 state.
        if self._is_transient_captcha_error(error_message):
            await self.db.reset_error_count(token_id)
            debug_logger.log_warning(
                f"[TOKEN_RECOVERY] Token {token_id} hit transient reCAPTCHA risk control; "
                "kept active and reset consecutive error counter"
            )
            return
