...
def _apply_omni_flash_model_updates():
    """Expose the current Google Flow Omni Flash (abra) model family."""
    landscape = "VIDEO_ASPECT_RATIO_LANDSCAPE"
    portrait = "VIDEO_ASPECT_RATIO_PORTRAIT"

    for seconds in (4, 6, 8, 10):
        suffix = f"{seconds}s"
        for orientation, aspect_ratio in (("landscape", landscape), ("portrait", portrait)):
            common = {
                "use_v2_model_config": True,
                "allow_tier_upgrade": False,
            }
            MODEL_CONFIG[f"omni_flash_t2v_{suffix}_{orientation}"] = _make_t2v_config(
                f"abra_t2v_{suffix}", aspect_ratio, **common
            )
            MODEL_CONFIG[f"omni_flash_i2v_{suffix}_{orientation}"] = _make_i2v_config(
                f"abra_i2v_{suffix}",
                aspect_ratio,
                min_images=1,
                max_images=1,
            ...
            MODEL_CONFIG[f"omni_flash_r2v_{suffix}_{orientation}"] = {
                "type": "video",
                "video_type": "r2v",
                "model_key": f"abra_r2v_{suffix}",
                "aspect_ratio": aspect_ratio,
                "supports_images": True,
                "min_images": 1,
                "max_images": 7,
    ...
    for orientation, aspect_ratio in (("landscape", landscape), ("portrait", portrait)):
        MODEL_CONFIG[f"omni_flash_edit_{orientation}"] = {
            "type": "video",
            "video_type": "edit",
            "model_key": "abra_edit",
            "aspect_ratio": aspect_ratio,
...
# Compatibility for V14.10.54 and earlier desktop builds, which emitted an
# explicit landscape segment for the default 8-second fast I2V model.
MODEL_CONFIG["veo_3_1_i2v_s_fast_landscape_fl"] = dict(
    MODEL_CONFIG["veo_3_1_i2v_s_fast_fl"]
)
    ...
    async def _handle_video_generation(
        ...
        try:
            ...
            # R2V: 多图生成 - 当前上游协议最多 3 张参考图
                ...
                if not video_bytes:
                    error_msg = "❌ Omni Flash 视频编辑需要上传源视频"
                    if stream:
                        yield self._create_stream_chunk(f"{error_msg}\n")
                    self._mark_generation_failed(generation_result, error_msg)
                    yield self._create_error_response(error_msg, status_code=400)
                ...
                if image_count > max_images:
                    error_msg = f"❌ Omni Flash 视频编辑最多支持 {max_images} 张参考图，当前提供了 {image_count} 张"
                    if stream:
                        yield self._create_stream_chunk(f"{error_msg}\n")
                    self._mark_generation_failed(generation_result, error_msg)
                    yield self._create_error_response(error_msg, status_code=400)
            ...
            elif video_type == "edit" and images:
                if stream:
                    yield self._create_stream_chunk(f"上传 {image_count} 张视频编辑参考图片...\n")
                for img in images:
                    media_id = await self.flow_client.upload_image(
                        token.at, img, model_config["aspect_ratio"], project_id=project_id
                    )
            ...
            try:
                ...
                if status == "MEDIA_GENERATION_STATUS_SUCCESSFUL":
                    ...
                    if not video_url:
                        ...
                        video_url = await self.flow_client.get_media_url(token.st, media_name)
                        if video_url:
