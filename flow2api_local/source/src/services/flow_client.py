...
class FlowClient:
    ...
    async def get_media_url(self, st: str, media_name: str) -> Optional[str]:
        """Resolve the signed source URL for media returned without an inline URL."""
        if not st or not media_name:
            return None

        url = f"{self.labs_base_url}/trpc/media.getMediaUrlRedirect?name={quote(str(media_name), safe='')}"

        def resolve_redirect() -> Optional[str]:
            ...
            request = urllib.request.Request(
                ...
                headers={
                    ...
                    "User-Agent": self._generate_user_agent(st[:16]),
                },
            )
            opener = urllib.request.build_opener(NoRedirectHandler())
            try:
                response = opener.open(request, timeout=self._get_video_poll_timeout())
        ...
        try:
            return await asyncio.to_thread(resolve_redirect)
        except Exception as exc:
            debug_logger.log_warning(f"[VIDEO] Failed to resolve media URL for {media_name}: {exc}")
            return None
    ...
    async def generate_video_edit(
        ...
        """Edit an uploaded video with Flow Omni Flash (abra_edit)."""
        url = f"{self.api_base_url}/video:batchAsyncGenerateVideoEditVideo"
        upload_base = f"{self.labs_base_url}/upload-video"

        def upload_source_video() -> str:
            # Flow accepts resumable uploads. Sending a whole video in one PUT
            # works for short clips but makes larger files prone to upstream 500s
            # and proxy timeouts. Keep the source bytes intact and upload in
            # ordered chunks so a failed transfer can retry only its current part.
            chunk_size = 8 * 1024 * 1024
            upload_timeout = max(900, int(self._get_video_submit_timeout() or 0))
            upload_retries = 3
            file_name = f"flow2api_edit_{int(time.time() * 1000)}.mp4"
            cookie = f"__Secure-next-auth.session-token={st}"
            start_headers = {
                "Cookie": cookie,
            ...
            start_request = urllib.request.Request(
                f"{upload_base}?action=start", data=b"", headers=start_headers, method="POST"
            )
            try:
                with urllib.request.urlopen(start_request, timeout=120) as response:
                    session_url = json.loads(response.read().decode("utf-8"))["sessionUrl"]
            except urllib.error.HTTPError as exc:
                detail = exc.read().decode("utf-8", "replace")[:1000]
                raise RuntimeError(f"Flow video upload session failed (HTTP {exc.code}): {detail}") from exc

            total_size = len(video_bytes)
            upload_result = {}
            for offset in range(0, total_size, chunk_size):
                chunk = video_bytes[offset:offset + chunk_size]
                is_final_chunk = offset + len(chunk) >= total_size
                command = "upload, finalize" if is_final_chunk else "upload"
                last_error = None
                for attempt in range(upload_retries):
                    debug_logger.log_info(
                        f"[VIDEO EDIT] Uploading source video chunk "
                        f"{offset // chunk_size + 1} at offset {offset} "
                        f"({len(chunk)} bytes, final={is_final_chunk}, attempt={attempt + 1})"
                    )
                    upload_headers = {
                        "Cookie": cookie,
                    ...
                    upload_request = urllib.request.Request(
                        f"{upload_base}?action=upload", data=chunk, headers=upload_headers, method="PUT"
                    )
                    try:
                        with urllib.request.urlopen(upload_request, timeout=upload_timeout) as response:
                            response_body = response.read().decode("utf-8", "replace")
                        if is_final_chunk:
                            upload_result = json.loads(response_body or "{}")
                        break
                    except urllib.error.HTTPError as exc:
                        detail = exc.read().decode("utf-8", "replace")[:1000]
                        last_error = RuntimeError(
                            f"Flow video upload chunk {offset // chunk_size + 1} failed "
                            f"(HTTP {exc.code}, attempt {attempt + 1}/{upload_retries}): {detail}"
                        )
                    except Exception as exc:
                        last_error = RuntimeError(
                            f"Flow video upload chunk {offset // chunk_size + 1} failed "
                            f"(attempt {attempt + 1}/{upload_retries}): {exc}"
                        )
                    if attempt + 1 < upload_retries:
                        time.sleep(1.5 * (attempt + 1))
                else:
                    raise last_error or RuntimeError("Flow video upload failed")

            media_id = upload_result.get("mediaServerId")
            if not media_id:
                raise RuntimeError(f"Flow video upload did not return mediaServerId: {upload_result}")
            return media_id

        video_media_id = await asyncio.to_thread(upload_source_video)
        debug_logger.log_info(f"[VIDEO EDIT] 源视频上传完成: {video_media_id}")
        max_retries = config.flow_max_retries
        last_error = None
        ...
        for retry_attempt in range(max_retries):
            ...
            if not launch_ok:
                raise Exception("Video launch queue wait timeout")
            try:
                recaptcha_token, browser_id = await self._get_recaptcha_token(
            ...
            json_data = {
                "mediaGenerationContext": self._build_video_media_generation_context(str(uuid.uuid4())),
                "clientContext": {
                    "recaptchaContext": {
                        "token": recaptcha_token,
                        "applicationType": "RECAPTCHA_APPLICATION_TYPE_WEB",
                ...
                "requests": [{
                    ...
                    "textInput": self._build_video_text_input(prompt, use_v2_model_config=False),
                    "metadata": {"sceneId": scene_id},
                }],
            }
            if reference_images:
                json_data["requests"][0]["referenceImages"] = reference_images
            ...
            try:
                ...
                if any(marker in error_text for marker in (
                    ...
                    raise RuntimeError(
                        "Google Flow 暂时限制当前账号或网络的视频生成（reCAPTCHA 风控）。"
                        "本次任务已停止，不会继续重复提交；请在 Flow 网页确认账号可生成视频并等待冷却后重试。"
                    ) from exc
    ...
    async def _handle_retryable_generation_error(
        ...
        unusual_activity = any(marker in error_str.lower() for marker in (
            "public_error_unusual_activity",
            "unusual_activity_too_much_traffic",
            "recaptcha evaluation failed",
        ))
        ...
        if config.captcha_method == "browser":
            ...
            try:
                ...
                await service.report_flow_error(
                    ...
                    error_reason=error_reason or "",
                    error_message=error_message or "",
                    slot_id=str(browser_id) if browser_id else None,
                )
            except Exception as e:
                debug_logger.log_warning(f"[reCAPTCHA Personal] 上报 Flow 错误失败: {e}")
    ...
    async def _get_recaptcha_token(
        ...
        Returns:
            ...
            - personal 模式: browser_id 为生成该 token 的精确工作器/标签页槽位 ID
        ...
        if captcha_method == "personal":
            ...
            try:
                # Prefer a connected Chrome worker. Its token is minted in the
                # same signed-in Flow tab the user just used successfully, so
                # project/session risk signals remain consistent. If no worker
                # is connected, retain the built-in browser as a fallback.
                from .browser_captcha_extension import ExtensionCaptchaService
                extension_service = await ExtensionCaptchaService.get_instance(self.db)
                has_extension, route_key = await extension_service.has_connection_for_token(token_id)
                if has_extension:
                    debug_logger.log_info(
                        f"[reCAPTCHA] 优先使用 Chrome Flow 会话: route_key={route_key or '-'}"
                    )
                    extension_timeout = 45 if action == "VIDEO_GENERATION" else 25
                    extension_token = await extension_service.get_token(
                        project_id,
                        action,
                        timeout=extension_timeout,
                        token_id=token_id,
                ...
                # the tab that minted the rejected reCAPTCHA token.
                return token, (_slot_id if callable(get_token_with_metadata) else None)
