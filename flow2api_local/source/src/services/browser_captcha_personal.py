        ...
        async def _recover_current_slot():
            if self._is_force_fresh_browser_restart_error(error_text):
                ...
                if resident_info is not None:
                    ...
                    # A full fresh-profile restart must wait for active requests to
                    # drain. Recreate the rejected tab immediately so the retry in
                    # this same request cannot mint another token from that slot.
