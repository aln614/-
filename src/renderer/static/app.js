...
let publicBatchUploadsActive = 0;
let publicBatchThumbRenderRaf = 0;
...
const APIMART_MODEL_OPTIONS = [
  ...
  ['gemini-3.1-flash-lite-image-ext','Gemini-3.1-Flash-Lite-Image Ext（Nano Banana Lite Ext）'],
  ...
  ['ltx-2.3-text-image','LTX 2.3 Text Image'],
  ...
  ['grok-imagine-2.0-ext','Grok Imagine 2.0 Ext'],
  ['grok-imagine-image-2.0','Grok Imagine Image 2.0 Official'],
...
const APIMART_IMAGE_MODEL_ALIASES = Object.freeze({
  'doubao-seedance-4-0': 'seedream-4.0',
...
function canonicalApimartImageModel(model){
  const value = imageModelKey(model);
  return APIMART_IMAGE_MODEL_ALIASES[value] || String(model || '').trim();
}
...
function isLtx23TextImageModel(model){ return imageModelKey(model) === 'ltx-2.3-text-image'; }
function isQwenImage3Model(model){
  return ['qwen-image-3.0','qwen-image-3.0-pro'].includes(imageModelKey(model));
}
function isQwenImage2Model(model){
  return ['qwen-image-2.0','qwen-image-2.0-pro'].includes(imageModelKey(model));
}
function isNanoBananaLiteModel(model){
...
function isGrokImagine2ExtModel(model){ return imageModelKey(model) === 'grok-imagine-2.0-ext'; }
function clampImageOutputCount(max){
  const imageN = $('#imageN');
  if(!imageN) return;
  const current = Number(imageN.value || 1);
  imageN.value = String(Math.max(1, Math.min(max, Number.isFinite(current) ? current : 1)));
}
function applyDocumentedImageUiGuard(model){
  if(currentImagePlatform() !== 'apimart') return;
  const m = imageModelKey(model);
  const sizeValue = getSizeValue();
  const setSizeIfUnsupported = (values, fallback='1:1') => {
    if(values.includes(sizeValue)) return;
    applySizeToUI(fallback);
  };
  const setResolution = (values, fallback) => {
    const clarity = $('#clarity');
    if(!clarity || values.includes(String(clarity.value || '').toUpperCase())) return;
    clarity.value = fallback;
  ...
  }else if(isLtx23TextImageModel(m)){
    clampImageOutputCount(1);
  }else if(isQwenImage2Model(m)){
    setSizeIfUnsupported(['1:1','4:3','3:4','16:9','9:16','3:2','2:3']);
    setResolution(['1K','2K'], '1K');
    clampImageOutputCount(6);
  }else if(isZImageTurboModel(m)){
    setSizeIfUnsupported(['1:1','4:3','3:4','16:9','9:16','3:2','2:3']);
    setResolution(['1K','2K'], '1K');
    clampImageOutputCount(1);
  }else if(isGrokImagine2ExtModel(m)){
    setSizeIfUnsupported(['1:1','2:3','3:2','3:4','4:3','9:16','16:9']);
    clampImageOutputCount(12);
  }else if(isGrokAspectRatioImageModel(m)){
    setSizeIfUnsupported(['auto','1:1','3:4','4:3','9:16','16:9','2:3','3:2','9:19.5','19.5:9','9:20','20:9','1:2','2:1']);
    setResolution(['1K','2K'], '1K');
...
function applyModelToUI(model){
  const v = currentImagePlatform() === 'apimart'
    ? (canonicalApimartImageModel(model) || 'gemini-3.1-flash-image-preview')
...
function updateSizeHint(){
  ...
  else if(isGrokImagine2ExtModel(model)) supportNote = '（Grok Imagine 2.0 Ext 仅支持文生图，固定质量模式，不提交 resolution）';
  else if(isGrokAspectRatioImageModel(model)) supportNote = '（Grok Imagine Image 使用 aspect_ratio 参数，支持 1K/2K 与参考图编辑）';
  else if(imageModelKey(model).includes('grok-imagine')) supportNote = '（Grok Imagine 1.5 最多 1 张参考图，不提交 resolution）';
  const sizeParam = isGrokAspectRatioImageModel(model) ? 'aspect_ratio' : 'size';
  const resolutionText = isGrokImagine2ExtModel(model) || String(model || '').startsWith('imagen-4.0') ? '不提交 resolution' : `resolution=${clarity}`;
  $('#sizeHint').textContent = `当前比例：${v}；APIMart 提交参数 ${sizeParam}=${actual}，${resolutionText}${supportNote}`;
}
...
const AGENT_PROGRAM_API_ALLOWLIST = new Set([
  ...
  '/api/assets/groups/create','/api/assets/groups/rename','/api/assets/groups/delete','/api/assets/groups/move','/api/assets/upload',
...
function cancelRecentVideoUploadPreview(id){
  const timer = recentVideoUploadPreviewTimers.get(id);
  if(timer) window.clearTimeout(timer);
  recentVideoUploadPreviewTimers.delete(id);
}
function closeRecentAudioPreview(){
  const player = $('#recentAudioPreviewPlayer');
  if(player){
    player.pause?.();
    player.removeAttribute('src');
    player.load?.();
  }
  const modal = $('#recentAudioPreviewModal');
  modal?.classList.remove('active');
  modal?.setAttribute('aria-hidden', 'true');
}
function showRecentAudioPreview(row={}){
  const source = recentUploadSourceUrl(row);
  if(!source){ toast('最近上传音频源文件不存在'); return; }
  const player = $('#recentAudioPreviewPlayer');
  const name = $('#recentAudioPreviewName');
  if(name) name.textContent = row.name || '最近上传音频';
  if(player){
    player.pause?.();
    player.src = source;
    player.load?.();
...
function previewRecentVideoUpload(id){
  const row = recentVideoUploadRowById(id);
  const source = row ? recentUploadSourceUrl(row) : '';
  if(!row || !source){ toast('最近上传素材源文件不存在'); return; }
  if(row.kind === 'video'){
    showVideoPreview({
      url:source,
      stream_url:source,
      download_url:source,
      status:'最近上传素材',
      progress_text:'单击预览，双击加入当前任务',
      model:'最近上传视频',
      filename:row.name || 'uploaded-video',
      created_at:row.created_at
    });
    return;
  }
  if(row.kind === 'audio'){
    showRecentAudioPreview(row);
    return;
  }
  showPreview(source, { model:'最近上传参考图', fullUrl:source, filename:row.name || 'uploaded-reference.png' });
}
function scheduleRecentVideoUploadPreview(id){
  cancelRecentVideoUploadPreview(id);
  const timer = window.setTimeout(()=>{
    recentVideoUploadPreviewTimers.delete(id);
    previewRecentVideoUpload(id);
  }, 220);
  recentVideoUploadPreviewTimers.set(id, timer);
}
...
function setupRecentUploadPanel(){
  ...
  if(videoPanel && videoPanel.dataset.bound !== '1'){
    ...
    videoPanel.addEventListener('dblclick', event=>{
      const item = event.target.closest('[data-recent-video-upload-id]');
      if(!item) return;
      event.preventDefault();
      cancelRecentVideoUploadPreview(item.dataset.recentVideoUploadId);
      addRecentVideoUploadToInputs(item.dataset.recentVideoUploadId);
    });
    videoPanel.addEventListener('keydown', event=>{
      ...
      event.preventDefault();
      cancelRecentVideoUploadPreview(item.dataset.recentVideoUploadId);
      previewRecentVideoUpload(item.dataset.recentVideoUploadId);
    });
  }
  ...
  $('#closeRecentAudioPreview')?.addEventListener('click', closeRecentAudioPreview);
  $('#recentAudioPreviewModal')?.addEventListener('click', event=>{ if(event.target === event.currentTarget) closeRecentAudioPreview(); });
...
function batchUploadStateMarkup(item){
  if(!isPublicClient) return '';
  if(item.batch_upload_state === 'done') return '<span class="batch-upload-state done">已上传</span>';
  if(item.batch_upload_state === 'failed') return '<span class="batch-upload-state failed">重试</span>';
  if(item.batch_upload_state === 'queued') return '<span class="batch-upload-state queued">等待上传</span>';
  return '<span class="batch-upload-state">上传中</span>';
}
function renderThumbList(sel, arr, type){
  $(sel).innerHTML = arr.map((it,i)=>`<div class="thumb-wrap"><img class="thumb draggable-generated-thumb" draggable="true" src="${it.data}" data-preview="${it.data}" data-full-url="${it.data}" data-id="input_${type}_${i}" data-name="${escapeHtml(it.name||'input-image.png')}">${batchUploadStateMarkup(it)}<button class="thumb-del" data-type="${type}" data-i="${i}">×</button></div>`).join('');
  ...
  $$(sel+' .thumb-del').forEach(btn => btn.addEventListener('click', (e)=>{
    ...
    const removed = btn.dataset.type === 'main' ? mainImages.splice(i,1) : refImages.splice(i,1);
    releaseBatchMediaItem(removed[0]);
    renderThumbs(); calcEstimate();
  }));
}
...
$('#clearMainImagesBtn')?.addEventListener('click',()=>{ releaseBatchMediaItems(mainImages); mainImages=[]; renderThumbs(); calcEstimate(); toast('主图已清空'); });
$('#clearRefImagesBtn')?.addEventListener('click',()=>{ releaseBatchMediaItems(refImages); refImages=[]; renderThumbs(); calcEstimate(); toast('参考图已清空'); });
$('#clearAllInputImagesBtn')?.addEventListener('click',()=>{ releaseBatchMediaItems(mainImages); releaseBatchMediaItems(refImages); mainImages=[]; refImages=[]; renderThumbs(); calcEstimate(); toast('全部图片已清空'); });
...
async function submitCurrentImageBatch(opts = {}){
  ...
  if(isLtx23TextImageModel($('#model')?.value || $('#modelPreset')?.value) && (mainImages.length || refImages.length)){
    const message = 'LTX 2.3 Text Image 仅支持文生图，请先清空主图和参考图';
...
function attachBatchMediaSource(item, source, objectUrl=''){
  if(!item || !source) return item;
  Object.defineProperty(item, '__batchMediaSource', {value:source, configurable:true});
  if(objectUrl) Object.defineProperty(item, '__batchPreviewObjectUrl', {value:objectUrl, configurable:true});
  return item;
}
function releaseBatchMediaItem(item){
  const objectUrl = item?.__batchPreviewObjectUrl;
  if(objectUrl){
    try{ URL.revokeObjectURL(objectUrl); }catch{}
    try{ delete item.__batchPreviewObjectUrl; }catch{}
  }
}
...
function schedulePublicBatchThumbRender(){
  ...
  publicBatchThumbRenderRaf = requestAnimationFrame(()=>{ publicBatchThumbRenderRaf=0; renderThumbs(); });
}
async function publicBatchMediaBlob(item){
  if(item?.__batchMediaSource instanceof Blob) return item.__batchMediaSource;
  const src = String(item?.data || '');
  if(!/^(?:data:|blob:)/i.test(src)) throw new Error('图片没有可上传的原始数据');
  const response = await fetch(src);
  if(!response.ok) throw new Error('读取待上传图片失败');
  return response.blob();
}
function drainPublicBatchUploadQueue(){
  while(publicBatchUploadsActive < PUBLIC_BATCH_UPLOAD_CONCURRENCY && publicBatchUploadQueue.length){
    const job = publicBatchUploadQueue.shift();
    if(job.item.batch_upload_id){ job.resolve(job.item.batch_upload_id); continue; }
    publicBatchUploadsActive += 1;
    job.item.batch_upload_state = 'uploading';
    schedulePublicBatchThumbRender();
    (async()=>{
      const blob = await publicBatchMediaBlob(job.item);
      const ret = await api(`/api/batch_media_upload?name=${encodeURIComponent(job.item.name || 'input.png')}`, {method:'POST',headers:{'Content-Type':job.item.type || blob.type || 'application/octet-stream'},body:blob});
      job.item.batch_upload_id = ret.upload_id || '';
      if(!job.item.batch_upload_id) throw new Error('素材预上传没有返回 upload_id');
      job.item.batch_upload_state = 'done';
      job.item.batch_upload_error = '';
      job.resolve(job.item.batch_upload_id);
    })().catch(error=>{
      job.item.batch_upload_state = 'failed';
      job.item.batch_upload_error = error.message || String(error);
      publicBatchUploadPromises.delete(job.item);
      job.reject(error);
    }).finally(()=>{
      publicBatchUploadsActive = Math.max(0, publicBatchUploadsActive - 1);
      schedulePublicBatchThumbRender();
      drainPublicBatchUploadQueue();
    });
  }
}
function stagePublicBatchMedia(item){
  if(!isPublicClient || !item) return Promise.resolve(item?.batch_upload_id || '');
  if(item.batch_upload_id) return Promise.resolve(item.batch_upload_id);
  const existing = publicBatchUploadPromises.get(item);
  if(existing) return existing;
  let resolveUpload;
  let rejectUpload;
  const promise = new Promise((resolve,reject)=>{ resolveUpload=resolve; rejectUpload=reject; });
  publicBatchUploadPromises.set(item, promise);
  item.batch_upload_state = 'queued';
  publicBatchUploadQueue.push({item, resolve:resolveUpload, reject:rejectUpload});
  schedulePublicBatchThumbRender();
  drainPublicBatchUploadQueue();
  return promise;
}
function prestagePublicBatchMedia(items=[]){
  if(!isPublicClient) return;
  for(const item of items) stagePublicBatchMedia(item).catch(()=>{});
}
async function ensurePublicBatchMediaUploaded(items=[], onProgress=()=>{}){
  const list = (items || []).filter(Boolean);
  let completed = list.filter(item=>item.batch_upload_id).length;
  onProgress(completed, list.length);
  const pending = list.filter(item=>!item.batch_upload_id);
  await Promise.all(pending.map(async item=>{
      await stagePublicBatchMedia(item);
      completed += 1;
      onProgress(completed, list.length);
  }));
  return list.map(item=>item.batch_upload_id).filter(Boolean);
}
...
function registerApimartVideoUiRules(items = []){
  items.forEach(item=>{
    ...
    APIMART_VIDEO_MODEL_RULES_UI[model.toLowerCase()] = {
      ...
      referenceVideoDurationRange:item.referenceVideoDurationRange || null,
      referenceVideoTotalDurationMin:Number(item.referenceVideoTotalDurationMin || 0),
      referenceVideoTotalDurationMax:Number(item.referenceVideoTotalDurationMax || 0),
      referenceVideoDurationPlusOutputMax:Number(item.referenceVideoDurationPlusOutputMax || 0),
      firstLastDurations:item.firstLastDurations || null,
      resolutionDurationRules:item.resolutionDurationRules || null,
      resolutionAspectRatios:item.resolutionAspectRatios || null,
      supportsImages:item.supportsImages !== false,
      supportsVideo:item.supportsVideo === true,
      videoDisallowsImages:item.videoDisallowsImages === true,
      promptMaxLength:Number(item.promptMaxLength || 0),
      maxVideoCount:Number(item.maxVideoCount || 1),
      ...
      outputDurationAtMostReferenceVideo:item.outputDurationAtMostReferenceVideo === true,
      supportsSourceAudioSetting:item.supportsSourceAudioSetting === true,
      promptOptionalWithMedia:item.promptOptionalWithMedia === true,
      promptOptionalModes:Array.isArray(item.promptOptionalModes) ? item.promptOptionalModes.map(String) : [],
      supportsDocumentReference:item.supportsDocumentReference === true,
      supportsLinkReference:item.supportsLinkReference === true,
      characterOrientation:item.characterOrientation === true,
      referenceVideoDurationRangeByOrientation:item.referenceVideoDurationRangeByOrientation || null,
      ...
      audioDisallowsImages:item.audioDisallowsImages === true,
      audioRequiresResolution:item.audioRequiresResolution || '',
      audioDisallowsLastFrame:item.audioDisallowsLastFrame === true,
      supportsGeneratedAudio:item.supportsGeneratedAudio === true,
      defaultGeneratedAudio:item.defaultGeneratedAudio !== false,
      forceGeneratedAudio:item.forceGeneratedAudio === true,
      lastFrameRequiresResolution:item.lastFrameRequiresResolution || '',
...
// APIMart official video documentation overrides. Keep this list aligned with the
...
registerApimartVideoUiRules([
  ...
  { model:'veo3.1-lite', label:'VEO3.1 Lite', resolutions:['720p','1080p','4k'], aspects:['16:9','9:16'], durations:[8], defaultDuration:8, supportsImages:false },
  ...
  { model:'veo3.1-quality-official', label:'VEO3.1 Official Quality', resolutions:['720p','1080p','4k'], defaultResolution:'720p', aspects:['16:9','9:16'], durations:[4,6,8], defaultDuration:8, maxImageCount:2 },
  { model:'flux-3-video', label:'FLUX 3 Video', resolutions:['hd','fhd'], defaultResolution:'hd', aspects:['auto','21:9','2:1','16:9','4:3','3:4','9:16'], defaultAspect:'auto', durationMin:5, durationMax:20, defaultDuration:5, supportsVideo:true, durationWithVideo:true, maxImageCount:10, maxVideoCount:1, note:'Supports text, 1-10 keyframe images, and one video continuation.' },
  ...
  { model:'kling-v3', label:'Kling v3', resolutions:['720p','1080p','4k'], aspects:['16:9','9:16','1:1'], durationMin:3, durationMax:15, defaultDuration:5, maxImageCount:2 },
  { model:'kling-v3-motion-control', label:'Kling v3 Motion Control', resolutions:['720p','1080p'], supportsAspect:false, supportsDuration:false, supportsVideo:true, requiredVideo:true, minImageCount:1, maxImageCount:1, characterOrientation:true, note:'Duration follows the source video; image orientation supports 3-10s, video orientation supports 3-30s.' },
  { model:'kling-v3-omni', label:'Kling v3 Omni', resolutions:['720p','1080p','4k'], aspects:['16:9','9:16','1:1'], durationMin:3, durationMax:15, defaultDuration:5, supportsVideo:true },
  { model:'kling-video-o1', label:'Kling Video O1', resolutions:['720p','1080p'], aspects:['16:9','9:16','1:1'], durations:[5,10], defaultDuration:5, maxImageCount:2 },
  { model:'kling-3.0-turbo', label:'Kling 3.0 Turbo', resolutions:['720p','1080p'], aspects:['16:9','9:16','1:1'], durationMin:3, durationMax:15, defaultDuration:5, maxImageCount:1 },
  { model:'viduq3', label:'Vidu Q3', resolutions:['540p','720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:3, durationMax:16, defaultDuration:5, minImageCount:1, maxImageCount:7 },
  { model:'viduq3-mix', label:'Vidu Q3 Mix', resolutions:['720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:1, durationMax:16, defaultDuration:5, minImageCount:1, maxImageCount:7 },
  { model:'viduq3-pro', label:'Vidu Q3 Pro', resolutions:['540p','720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:1, durationMax:16, defaultDuration:5, maxImageCount:2 },
  { model:'viduq3-turbo', label:'Vidu Q3 Turbo', resolutions:['540p','720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:1, durationMax:16, defaultDuration:5, maxImageCount:2 },
  { model:'grok-imagine-1.5-video-apimart', label:'Grok Imagine 1.5 Video', resolutions:['480p','720p'], defaultResolution:'480p', aspects:['16:9','9:16','1:1','3:2','2:3'], durationMin:6, durationMax:30, defaultDuration:6, maxImageCount:7 },
...
// Documentation deltas added after the initial series rollout. This final layer is
...
registerApimartVideoUiRules([
  ...
  { model:'kling-v2-6', label:'Kling 2.6', resolutions:['720p','1080p'], aspects:['16:9','9:16','1:1'], durations:[5,10], defaultDuration:5, maxImageCount:2, lastFrameRequiresResolution:'1080p', audioRequiresResolution:'1080p', audioDisallowsLastFrame:true, note:'Two-image first/last-frame and generated audio both require 1080p Pro; they cannot be combined.' },
  { model:'kling-v2-6-motion-control', label:'Kling 2.6 Motion Control', resolutions:['720p','1080p'], supportsAspect:false, supportsDuration:false, supportsVideo:true, requiredVideo:true, minImageCount:1, maxImageCount:1, characterOrientation:true, referenceVideoDurationRangeByOrientation:{image:[3,10],video:[3,30]} },
  { model:'kling-v3-motion-control', label:'Kling v3 Motion Control', resolutions:['720p','1080p'], supportsAspect:false, supportsDuration:false, supportsVideo:true, requiredVideo:true, minImageCount:1, maxImageCount:1, characterOrientation:true, referenceVideoDurationRangeByOrientation:{image:[3,10],video:[3,30]} },
  { model:'kling-v3-omni', label:'Kling v3 Omni', resolutions:['720p','1080p','4k'], aspects:['16:9','9:16','1:1'], durationMin:3, durationMax:15, defaultDuration:5, supportsVideo:true, audioDisallowsVideo:true },
  { model:'kling-video-o1', label:'Kling Video O1', resolutions:['720p','1080p'], aspects:['16:9','9:16','1:1'], durations:[5,10], defaultDuration:5, maxImageCount:2, supportsVideo:true },
  { model:'viduq3-pro', label:'Vidu Q3 Pro', resolutions:['540p','720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:1, durationMax:16, defaultDuration:5, maxImageCount:2, promptOptionalWithMedia:true },
  { model:'viduq3-turbo', label:'Vidu Q3 Turbo', resolutions:['540p','720p','1080p'], aspects:['16:9','9:16','4:3','3:4','1:1'], durationMin:1, durationMax:16, defaultDuration:5, maxImageCount:2, promptOptionalWithMedia:true },
  { model:'grok-imagine-1.5-video-apimart', label:'Grok Imagine 1.5 Video', resolutions:['480p','720p'], defaultResolution:'480p', aspects:['16:9','9:16','1:1','3:2','2:3'], durationMin:6, durationMax:15, defaultDuration:6, maxImageCount:7 },
  { model:'grok-imagine-video', label:'Grok Imagine Video Official', resolutions:['480p','720p'], defaultResolution:'480p', aspects:['auto','1:1','16:9','9:16','4:3','3:4','3:2','2:3'], defaultAspect:'auto', durationMin:1, durationMax:15, defaultDuration:8, supportsVideo:true, videoDisallowsImages:true, promptMaxLength:8000, note:'支持文生、参考图生成和单视频编辑；视频编辑自动跟随源视频规格。' },
  { model:'grok-imagine-video-1.5', label:'Grok Imagine Video 1.5 Official', resolutions:['480p','720p','1080p'], defaultResolution:'480p', aspects:['auto','1:1','16:9','9:16','4:3','3:4','3:2','2:3'], defaultAspect:'auto', durationMin:1, durationMax:15, defaultDuration:8, promptMaxLength:8000, note:'支持文生和参考图生成，不支持上传视频编辑。' },
  { model:'pixverse-v6', label:'Pixverse v6', resolutions:['360p','540p','720p','1080p'], defaultResolution:'540p', aspects:['16:9','4:3','1:1','3:4','9:16','2:3','3:2','21:9'], durationMin:1, durationMax:15, defaultDuration:5, firstLastDurations:[5,8], maxImageCount:7 }
]);
// Per-model media limits and generated-audio behavior from the current APIMart
// documentation. These patch the normalized UI entries above without reintroducing
// stale model variants into the picker.
function applyApimartVideoUiDocumentDeltas(items=[]){
  items.forEach(item=>{
    const key = String(item?.model || '').toLowerCase();
    const rule = APIMART_VIDEO_MODEL_RULES_UI[key];
    if(rule) Object.assign(rule, item);
  });
}
applyApimartVideoUiDocumentDeltas([
  {model:'gemini-omni-flash-preview', referenceVideoDurationRange:[1,24]},
  {model:'omni-flash-ext', allowedImageCounts:[0,1,3], maxVideoCount:1},
  {model:'doubao-seedance-1-5-pro', supportsGeneratedAudio:true, defaultGeneratedAudio:true},
  {model:'doubao-seedance-2.0', supportsGeneratedAudio:true, defaultGeneratedAudio:true, referenceVideoTotalDurationMin:1.8, referenceVideoTotalDurationMax:15.2, disallowFrameReferenceMix:true},
  {model:'doubao-seedance-2.0-fast', supportsGeneratedAudio:true, defaultGeneratedAudio:true, referenceVideoTotalDurationMin:1.8, referenceVideoTotalDurationMax:15.2, disallowFrameReferenceMix:true},
  {model:'doubao-seedance-2.0-mini', supportsGeneratedAudio:true, defaultGeneratedAudio:true, referenceVideoTotalDurationMin:1.8, referenceVideoTotalDurationMax:15.2, disallowFrameReferenceMix:true},
  {model:'doubao-seedance-2.5', supportsGeneratedAudio:true, defaultGeneratedAudio:true, referenceVideoDurationRange:[2,30], referenceVideoTotalDurationMax:30},
  {model:'veo3.1-fast-official', supportsGeneratedAudio:true, defaultGeneratedAudio:false},
  {model:'veo3.1-quality-official', supportsGeneratedAudio:true, defaultGeneratedAudio:false},
  {model:'flux-3-video', supportsGeneratedAudio:true, defaultGeneratedAudio:true},
  {model:'MiniMax-H3', referenceVideoDurationRange:[2,15], referenceVideoTotalDurationMax:15, audioMinDuration:2},
  {model:'happyhorse-1.0', promptOptionalWithMedia:false, promptOptionalModes:['first_frame'], referenceVideoDurationRange:[3,60], supportsSourceAudioSetting:true},
  {model:'happyhorse-1.1', promptOptionalWithMedia:false},
  {model:'wan2.5-preview', supportsGeneratedAudio:true, defaultGeneratedAudio:true, forceGeneratedAudio:true},
  {model:'wan2.6', supportsGeneratedAudio:true, defaultGeneratedAudio:true},
  {model:'wan2.6-i2v-flash', supportsGeneratedAudio:true, defaultGeneratedAudio:true},
  {model:'wan2.7', allowedImageCounts:[0,1,2], maxImageCount:2, referenceVideoDurationRange:[2,10]},
  {model:'wan2.7-r2v', referenceVideoDurationRange:[1,30]},
  {model:'wan2.7-videoedit', promptOptionalWithMedia:true, referenceVideoDurationRange:[2,10], outputDurationAtMostReferenceVideo:true, supportsSourceAudioSetting:true},
  {model:'kling-v2-6', supportsGeneratedAudio:true, defaultGeneratedAudio:false},
  {model:'kling-v2-6-motion-control', promptOptionalWithMedia:true},
  {model:'kling-v3', supportsGeneratedAudio:true, defaultGeneratedAudio:false},
  {model:'kling-v3-motion-control', promptOptionalWithMedia:true},
  {model:'kling-v3-omni', supportsGeneratedAudio:true, defaultGeneratedAudio:false, referenceVideoDurationRange:[3,10]},
  {model:'kling-video-o1', referenceVideoDurationRange:[3,10]},
  {model:'viduq3-pro', supportsGeneratedAudio:true, defaultGeneratedAudio:true},
...
function updateWan3ReferenceControls(){
  ...
  const row = $('#wan3ReferenceRow');
  const documentInput = $('#wan3DocumentFile');
  const linkInput = $('#wan3LinkUrl');
  const autoDuration = $('#wan3AutoDuration');
  if(row) row.classList.toggle('hidden', !enabled);
  if(documentInput) documentInput.disabled = !enabled;
  if(linkInput) linkInput.disabled = !enabled;
...
async function handleWan3DocumentFile(files){
  const file = [...(files || [])][0];
  if(!file) return;
  const ext = String(file.name || '').match(/\.[^.]+$/)?.[0]?.toLowerCase() || '';
  const allowed = new Set(['.doc','.docx','.xls','.xlsx','.ppt','.pptx','.pdf','.txt','.key','.pages','.numbers','.md']);
...
function updateVideoGeneratedAudioControl(){
  const row = $('#videoGeneratedAudioRow');
  const input = $('#videoGenerateAudio');
  const help = $('#videoGeneratedAudioHelp');
  const note = $('#videoGeneratedAudioRule');
  if(!row || !input) return;
  const rule = currentApimartVideoRule();
  const isDedicatedSeedanceControl = isSeedance25VideoModel();
  const supported = currentVideoPlatform() === 'apimart' && rule.supportsGeneratedAudio === true && !isDedicatedSeedanceControl;
  row.classList.toggle('hidden', !supported);
  if(!supported) return;
  const modelKey = String($('#videoModel')?.value || '').toLowerCase();
  if(input.dataset.audioModel !== modelKey){
    input.checked = rule.defaultGeneratedAudio !== false;
    input.dataset.audioModel = modelKey;
  }
  const hasVideo = hasReferenceVideo();
  const forced = rule.forceGeneratedAudio === true;
  const blockedByVideo = rule.audioDisallowsVideo === true && hasVideo;
  input.checked = forced ? true : input.checked;
  input.disabled = forced || blockedByVideo;
  input.title = forced
    ? '当前模型仅支持生成音频。'
    : (blockedByVideo ? '当前模型使用参考视频时不能同时生成音频。' : '');
  if(help) help.textContent = forced
    ? '当前模型的音频必须开启。'
    : (blockedByVideo ? '上传参考视频时，该模型不能同时生成音频。' : '可关闭以生成静音视频。');
  if(note){
    const limits = [];
    if(rule.audioRequiresResolution) limits.push(`仅 ${rule.audioRequiresResolution}`);
    if(rule.audioDisallowsLastFrame) limits.push('不能与首尾帧并用');
    if(rule.audioDisallowsVideo) limits.push('不能与参考视频并用');
    note.textContent = limits.length ? `生成音频：${limits.join('；')}。` : '生成音频规则已按当前模型配置。';
  }
}
function updateVideoSourceAudioSettingControl(){
  const row = $('#videoSourceAudioSettingRow');
  const input = $('#videoSourceAudioSetting');
  const note = $('#videoSourceAudioSettingNote');
  if(!row || !input) return;
  const rule = currentApimartVideoRule();
  const supported = currentVideoPlatform() === 'apimart' && rule.supportsSourceAudioSetting === true && hasReferenceVideo();
  row.classList.toggle('hidden', !supported);
  input.disabled = !supported;
  if(!['auto','origin'].includes(input.value)) input.value = 'auto';
  if(note) note.textContent = supported
    ? '自动：AI 根据编辑后画面重建匹配音频；保留原音轨：强制保留源视频声音。'
    : '';
}
...
function currentVideoDurationSpec(){
  if(currentVideoPlatform() === 'apimart'){
    ...
    if(wan3UsesAutomaticDuration()){
      return { values:[], defaultValue:-1, mode:'auto', note:'Wan3.0 will select the output duration automatically.' };
    }
    ...
    if(hasReferenceVideo() && Array.isArray(rule.videoDurationRange)){
      values = videoRange(Number(rule.videoDurationRange[0]), Number(rule.videoDurationRange[1]));
      mode = 'range';
    }
    if((currentVideoModeValue() === 'first_last_frame' || (currentVideoModeValue() === 'auto' && videoRefImages.length === 2)) && Array.isArray(rule.firstLastDurations)){
      values = normalizedVideoDurationValues(rule.firstLastDurations);
      mode = 'discrete';
    }
    const resolutionDurations = rule.resolutionDurationRules && rule.resolutionDurationRules[$('#videoResolution')?.value || ''];
    if(Array.isArray(resolutionDurations)){
      values = normalizedVideoDurationValues(resolutionDurations);
      mode = 'discrete';
    }
    return {
      values,
...
function updateVideoDurationDisplay(){
  ...
  if(values.length > 6){
    supportLabel.textContent = values[0] === 0 && values[1] === 2
      ? `可选 0 或 2–${values[values.length - 1]} 秒`
...
function updateVideoResolutionOptions(){
  ...
  if(currentVideoPlatform() === 'apimart'){
    ...
    if(aspect){
      ...
      const aspects = rule.resolutionAspectRatios?.[resolution.value] || rule.aspects;
      const fallbackAspect = aspects.includes(rule.defaultAspect) ? rule.defaultAspect : aspects[0];
      aspect.innerHTML = aspects.map(v=>`<option value="${v}">${v}</option>`).join('');
      aspect.value = [...aspect.options].some(o=>o.value===oldAspect) ? oldAspect : fallbackAspect;
    }
    return;
  }
...
function updateVideoModeUI(){
  ...
  if(selectedMode === 'veo_remix') mode = '任务续写';
  ...
  const modelRuleNote = $('#videoAdvancedRuleRow .setting-note-box');
  if(modelRuleNote && platform === 'apimart'){
  ...
  if($('#videoPlatformWarning')) $('#videoPlatformWarning').textContent = platform === 'flow2api'
    ...
    : isApimartOmniVideoModel()
      ...
      : isWan3VideoModel()
        ? 'Wan3.0 支持文生、首尾帧、多图/视频/音频参考，以及单个文档或公开网页参考；文档与网页不能同时使用。'
...
function setVideoApiPlatform(platform='apimart', silent=false){
  const p = platform === 'flow2api' ? 'flow2api' : 'apimart';
  const remixOption = $('#videoModeSelect option[value="veo_remix"]');
  if(remixOption) remixOption.hidden = p !== 'apimart';
  if(p !== 'apimart' && $('#videoModeSelect')?.value === 'veo_remix') $('#videoModeSelect').value = 'auto';
...
function clearVideoInputs(){
  ...
  $('#videoUrlInput').value='';
  if($('#wan3DocumentFile')) $('#wan3DocumentFile').value='';
  if($('#wan3LinkUrl')) $('#wan3LinkUrl').value='';
  if($('#wan3AutoDuration')) $('#wan3AutoDuration').checked=false;
  if($('#videoSourceTaskId')) $('#videoSourceTaskId').value='';
  updateWan3ReferenceControls();
  renderVideoInputs();
}
async function submitVideoTask(opts = {}){
  ...
  if(wan3DocumentFile && wan3LinkUrl) return reject('Wan3.0 文档参考和网页参考不能同时使用');
  ...
  if(apimartRule){
    ...
    if(refVideoMode && apimartRule.disallowFrameReferenceMix && ['first_frame','first_last_frame'].includes(selectedVideoMode)) return reject(`${apimartRule.label} 的首帧 / 首尾帧模式不能与参考视频同时使用，请切换为自动识别或多素材生成`);
    if(refVideoMode && apimartRule.videoMaxImageCount > 0 && videoRefImages.length > apimartRule.videoMaxImageCount) return reject(`${apimartRule.label} 的视频编辑最多支持 ${apimartRule.videoMaxImageCount} 张参考图`);
    const sourceVideoDurations = videoFilesData.map(item=>Number(item.duration_seconds)).filter(value=>Number.isFinite(value) && value > 0);
    if(sourceVideoDurations.length){
      const range = apimartRule.referenceVideoDurationRange;
      if(Array.isArray(range) && sourceVideoDurations.some(value=>value < Number(range[0]) || value > Number(range[1]))) return reject(`${apimartRule.label} 的参考视频时长需为 ${range[0]}-${range[1]} 秒`);
      const sourceVideoTotal = sourceVideoDurations.reduce((sum,value)=>sum + value, 0);
      const isCombinedReference = videoMultiReferenceEnabled() || sourceVideoDurations.length === 1;
      if(isCombinedReference && apimartRule.referenceVideoTotalDurationMin > 0 && sourceVideoTotal < apimartRule.referenceVideoTotalDurationMin) return reject(`${apimartRule.label} 的参考视频总时长需大于 ${apimartRule.referenceVideoTotalDurationMin} 秒`);
      if(isCombinedReference && apimartRule.referenceVideoTotalDurationMax > 0 && sourceVideoTotal > apimartRule.referenceVideoTotalDurationMax) return reject(`${apimartRule.label} 的参考视频总时长不能超过 ${apimartRule.referenceVideoTotalDurationMax} 秒`);
      const selectedDuration = selectedVideoDuration();
    ...
    if(activeAudioFiles.length){
      ...
      if(apimartRule.audioDisallowsImages && videoRefImages.length) return reject(`${apimartRule.label} 的自定义音频不能与参考图同时使用`);
    ...
    const isFirstLastFrameMode = selectedVideoMode === 'first_last_frame' || (selectedVideoMode === 'auto' && videoRefImages.length === 2);
    const generatedAudio = apimartRule.supportsGeneratedAudio === true && (seedance25
      ? $('#seedance25GenerateAudio')?.checked !== false
      : (apimartRule.forceGeneratedAudio === true || $('#videoGenerateAudio')?.checked === true));
    if(generatedAudio){
      const resolution = String($('#videoResolution')?.value || '').toLowerCase();
      if(apimartRule.audioRequiresResolution && resolution !== String(apimartRule.audioRequiresResolution).toLowerCase()) return reject(`${apimartRule.label} 生成音频仅支持 ${apimartRule.audioRequiresResolution}`);
      if(apimartRule.audioDisallowsLastFrame && isFirstLastFrameMode) return reject(`${apimartRule.label} 的生成音频不能与首尾帧同时使用`);
      if(apimartRule.audioDisallowsVideo && refVideoMode) return reject(`${apimartRule.label} 的生成音频不能与参考视频同时使用`);
    }
...
function setupVideoPage(){
  const modeSelect = $('#videoModeSelect');
  if(modeSelect && ![...modeSelect.options].some(option=>option.value === 'veo_remix')) modeSelect.insertAdjacentHTML('beforeend', '<option value="veo_remix">任务续写（VEO3 / Pixverse / Gemini）</option>');
  ...
  $('#videoModel')?.addEventListener('change', ()=>{ if($('#videoDuration')) $('#videoDuration').dataset.durationValue=''; updateVideoResolutionOptions(); updateVideoDurationOptions(); updateVideoDurationVisibility(); updateVideoModeUI(); updateVideoTaskEstimate(); });
  ...
  $('#videoResolution')?.addEventListener('change', ()=>{ updateVideoResolutionOptions(); updateVideoDurationOptions(); updateVideoModeUI(); updateVideoTaskEstimate(); });
  ...
  $('#wan3LinkUrl')?.addEventListener('input', ()=>{ updateVideoModeUI(); updateVideoTaskEstimate(); });
  ...
  $('#videoMultiReference')?.addEventListener('change', ()=>{
    updateVideoMultiReferenceControl();
    updateVideoTaskEstimate();
  });
...
function assetGroupDropAllowed(sourceId, parentId=''){
  ...
  if(assetDescendantGroupIds(sourceId).has(parentId)) return false;
  const parent=assetGroupById(parentId);
  return !!parent.id && assetCanEdit(parent);
}
function assetGroupDropIntentForEvent(event,row,sourceId){
  if(!sourceId) return null;
  if(!row) return assetGroupDropAllowed(sourceId,'') ? {sourceId,targetId:'',parentId:'',beforeId:'',mode:'root'} : null;
  const targetId=String(row.dataset.id||'');
  if(!targetId || targetId===sourceId) return null;
  const target=assetGroupById(targetId);
  ...
  if(ratio<.3){
    mode='before';
    parentId=String(target.parent_id||'');
    beforeId=targetId;
  }else if(ratio>.7){
    mode='after';
    parentId=String(target.parent_id||'');
    const siblings=(assetState.groups||[]).filter(group=>group.id!==sourceId && String(group.parent_id||'')===parentId).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
...
function markAssetGroupDropTarget(row,intent){
...
function assetRecursiveCountMap(){
  ...
  (assetState.groups || []).forEach(g=>{
    const parentId=String(g.parent_id||'');
    if(!children.has(parentId)) children.set(parentId,[]);
    children.get(parentId).push(g);
  });
  const memo = new Map();
  const count = (id)=>{
    if(memo.has(id)) return memo.get(id);
    let n = direct.get(id) || 0;
    (children.get(String(id||'')) || []).forEach(g=>{ n += count(g.id); });
    memo.set(id, n);
    return n;
  };
...
let assetTextMeasureContext=null;
const assetTextFitCache=new Map();
function assetBestTextSize(text,width,minSize,maxSize,lines,fontFamily,fontWeight){
  if(!assetTextMeasureContext){
    const canvas=document.createElement('canvas');
    assetTextMeasureContext=canvas.getContext('2d');
  }
  if(!assetTextMeasureContext) return minSize;
  const cacheKey=[text,Math.round(width),minSize,maxSize,lines,fontWeight,fontFamily].join('|');
  if(assetTextFitCache.has(cacheKey)) return assetTextFitCache.get(cacheKey);
  let low=minSize, high=maxSize, best=minSize;
  for(let i=0;i<6;i++){
    const size=(low+high)/2;
    assetTextMeasureContext.font=`${fontWeight} ${size}px ${fontFamily}`;
    const usableWidth=Math.max(1,width-size);
    const textWidth=assetTextMeasureContext.measureText(String(text||'')).width;
    if(textWidth<=usableWidth*lines){
      best=size;
      low=size;
    }else{
      high=size;
  ...
  if(assetTextFitCache.size>2400) assetTextFitCache.clear();
  assetTextFitCache.set(cacheKey,best);
  return best;
}
function fitAssetTextElement(target,fontFamily){
  const {el,minSize,maxSize,lines,width,text}=target;
  if(!el || el.querySelector('input') || !width) return;
  const fontWeight=el.classList.contains('asset-name')?900:800;
  const size=assetBestTextSize(text,width,minSize,maxSize,lines,fontFamily,fontWeight);
  el.style.fontSize=`${size}px`;
  el.style.lineHeight='1.2';
  el.style.height=`${Math.ceil(size*1.2*lines)}px`;
}
function scheduleAssetTextFit(includeGroups=true){
  if(assetTextFitRaf) cancelAnimationFrame(assetTextFitRaf);
  assetTextFitRaf=requestAnimationFrame(()=>{
    assetTextFitRaf=0;
    const targets=$$('#assetGrid .asset-name').map(el=>({el,minSize:9.5,maxSize:13,lines:2,width:el.clientWidth,text:el.textContent||''}));
    if(includeGroups) targets.push(...$$('#assetGroupTree .asset-group-name').map(el=>({el,minSize:9.5,maxSize:13,lines:3,width:el.clientWidth,text:el.textContent||''})));
    const fontFamily=getComputedStyle(document.body).fontFamily||'sans-serif';
    targets.forEach(target=>fitAssetTextElement(target,fontFamily));
  });
}
...
function assetSortRowsForView(rows=[]){
...
function assetApplyLoadedFilter(){
  if(!assetState.currentGroup){
    assetState.assets=[];
  ...
  if(tree && !skipTree){
    ...
    const renderSection = (title, subtitle, predicate, showHeader=true) => {
      ...
      sectionGroups.forEach(group=>{
        const rawParent=String(group.parent_id||'');
        const parentId=rawParent && ids.has(rawParent) ? rawParent : '';
        if(!byParent.has(parentId)) byParent.set(parentId,[]);
        byParent.get(parentId).push(group);
      });
      byParent.forEach(list=>list.sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0)));
      ...
      const walk = (parent='', level=0) => {
        (byParent.get(String(parent||''))||[]).forEach(g=>{
          ...
          const hasChildren = (byParent.get(String(g.id||''))||[]).length>0;
          const collapsed = hasChildren && assetState.collapsedGroups.has(g.id);
          const draggable = !readonly && assetCanEdit(g);
...
async function assetRenameAsset(id, name){
  const clean = String(name || '').trim();
  if(!id || !clean) return;
  const ret=await api('/api/assets/rename',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,name:clean})});
  if(ret.asset){
    const index=assetState.allAssets.findIndex(asset=>asset.id===id);
    if(index>=0) assetState.allAssets[index]=ret.asset;
  }
...
async function assetReorderAsset(id, beforeId=''){
  ...
  const siblings=assetSortRowsForView(assetState.allAssets.filter(asset=>asset.group_id===source.group_id && asset.id!==source.id));
  const insertAt=before ? Math.max(0,siblings.findIndex(asset=>asset.id===before.id)) : siblings.length;
  siblings.splice(insertAt<0?siblings.length:insertAt,0,source);
...
let assetMediaObserver=null;
function assetLoadCardMedia(node){
  if(!node || node.dataset.mediaReady) return;
  const source=node.dataset.assetMediaSrc||'';
  if(!source) return;
  node.dataset.mediaReady='1';
  node.removeAttribute('data-asset-media-src');
  if(node.matches('img')){
    node.src=source;
    return;
  }
...
function assetHydrateCardMedia(){
  requestAnimationFrame(()=>{
    assetMediaObserver?.disconnect();
    assetMediaObserver=null;
    const grid=$('#assetGrid');
    const media=$$('#assetGrid [data-asset-media-src]');
    if(!media.length) return;
    const immediate=media.slice(0,12);
    immediate.forEach(assetLoadCardMedia);
    const deferred=media.slice(immediate.length);
    if(!deferred.length) return;
    if(!window.IntersectionObserver){
      deferred.forEach(assetLoadCardMedia);
      return;
    }
    assetMediaObserver=new IntersectionObserver(entries=>{
      entries.forEach(entry=>{
        if(!entry.isIntersecting) return;
        assetMediaObserver?.unobserve(entry.target);
        assetLoadCardMedia(entry.target);
      });
    },{root:grid,rootMargin:'240px 0px',threshold:0.01});
    deferred.forEach(node=>assetMediaObserver.observe(node));
  });
}
...
function setupAssetLibrary(){
  ...
  $('#assetGroupTree')?.addEventListener('click',async e=>{
    if(assetState.draggingGroupId || Date.now()-assetState.groupDragJustEnded<220) return;
    ...
    assetState.selected.clear();
    if($('#assetGrid')) $('#assetGrid').scrollTop=0;
    await loadAssetAssets();
  });
  ...
  $('#assetClearSelectionBtn')?.addEventListener('click',()=>{assetState.selected.clear(); renderAssetLibrary({skipTree:true});});
